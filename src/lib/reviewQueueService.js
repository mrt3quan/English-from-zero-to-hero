...
const KEY='bunny-english.review-queue.v2'
const LEGACY_KEY='bunny-english.review-queue.v1'
...
export const ReviewQueueService = {
  list(){
    ...
    const migrated=(legacy||[]).filter(item=>item.type!=='lesson_review').map(item=>({...item,migratedFrom:'v1'}))
  ...
  addMistake({lessonId,stepId,skillIds,prompt,answer,expected,errorTags,task}){
    return this.upsert({key:`mistake:${lessonId}:${stepId}`,type:'mistake',lessonId,stepId,skillIds,prompt,answer,expected,errorTags,task,dueAt:new Date().toISOString(),lastRating:0})
  },
  ...
  clear(){ removeKey(KEY); removeKey(LEGACY_KEY); this.emit() },
