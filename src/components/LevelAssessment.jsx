...
import { BookOpen, Check, CheckCircle2, ChevronLeft, ChevronRight, Circle, Headphones, Lightbulb, Mic, MicOff, PenLine, RotateCcw, Save, Trophy, Volume2, X } from 'lucide-react'
...
function SpeakingItem({item,state={},onChange}){
  ...
  const startBrowser=()=>{
    ...
    SpeechService.listen({locale:'en-US',onResult:({transcript})=>{setBusy(false);onChange({...state,completed:true,skipped:false,transcript,score:scoreTranscript(transcript,item.target),provider:'browser-speech'})},onError:()=>{setBusy(false);setError('Bunny chưa nghe rõ. Bạn có thể thử lại hoặc bỏ qua phần nói.')}})
  }

  const assess=async blob=>{
    setRecording(false);setBusy(true);setError('')
    try{
      const result=await PronunciationService.assess({audio:blob,target:item.target,accent:'GA',l1:'vi'})
      onChange({...state,completed:true,skipped:false,transcript:result.transcript,pronunciation:result,provider:'pronounce-ai'})
    }catch{
      setError('Bộ chấm phát âm đang tạm không kết nối được. Bạn vẫn có thể dùng kiểm tra từ hoặc bỏ qua phần nói.')
    }finally{setBusy(false)}
  }

  const startPronunciation=async()=>{
    PronunciationService.prewarm({phrase:item.target,accent:'GA'})
    const rec=await AudioRecorderService.start({maxDurationMs:10000,onStart:()=>setRecording(true),onStop:({blob})=>assess(blob),onError:message=>{setRecording(false);setError(message==='not-allowed'?'Microphone đang bị chặn. Bạn có thể bỏ qua phần nói.':'Không thể bắt đầu ghi âm.')}})
    controller.current=rec
  }

  const start=()=>pronunciationReady?startPronunciation():startBrowser()
  const skip=()=>{controller.current?.stop?.();pauseSpeaking();setPaused(true);setBusy(false);setRecording(false);onChange({...state,completed:true,skipped:true,skipReason:'cant-talk-right-now'})}
  const resume=()=>{resumeSpeaking();setPaused(false);setError('')}
  const feedback=state.pronunciation?vietnamesePronunciationFeedback(state.pronunciation):''

  return <div className="assessment-question rounded-3xl p-5"><p className="text-sm font-bold text-strong">{item.promptVi}</p><p className="mt-2 text-lg font-extrabold">{item.target}</p>
    {paused?<div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3"><div className="flex items-start gap-2"><MicOff className="mt-0.5 h-4 w-4 text-slate-500"/><div><p className="text-xs font-extrabold text-slate-700">Bạn đang tạm nghỉ luyện nói</p><p className="mt-1 text-[11px] font-semibold leading-5 text-slate-500">Còn khoảng {formatSpeakingPauseRemaining()||'15 phút'}. Phần nói A0 chỉ là luyện tập và không làm bạn rớt bài kiểm tra.</p></div></div><button onClick={resume} className="mt-2 min-h-9 rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold text-slate-700">Tôi có thể nói lại</button></div>:
    <><div className="mt-3 flex flex-wrap gap-2"><button disabled={busy} onClick={recording?()=>controller.current?.stop?.():start} className="pressable inline-flex min-h-11 items-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-bold text-white disabled:opacity-40"><Mic className="h-4 w-4"/>{recording?'Đang ghi… chạm để chấm':busy?'Bunny đang nghe lại…':pronunciationReady?'Nói để Bunny kiểm tra':'Thử microphone'}</button><button onClick={()=>onChange({...state,completed:true,selfConfirmed:true,skipped:false})} className="pressable min-h-11 rounded-xl border border-violet-200 bg-white px-4 text-sm font-bold text-violet-700">Tôi đã nói thành tiếng</button></div><button onClick={skip} className="mt-2 inline-flex min-h-9 items-center gap-2 rounded-xl px-2 text-xs font-bold text-slate-500 hover:bg-slate-50"><MicOff className="h-4 w-4"/> Tôi không thể nói lúc này</button></>}
    {state.pronunciation&&<div className="mt-3 rounded-2xl border border-violet-100 bg-violet-50 p-3"><p className="text-xs font-extrabold text-violet-900">{feedback}</p>{state.pronunciation.needsWork?.length>0&&<p className="mt-1 text-[11px] font-semibold text-violet-700">Âm cần chú ý: {state.pronunciation.needsWork.slice(0,3).map(x=>`/${x.expected||x.phoneme}/`).join(', ')}</p>}</div>}
    {state.transcript&&!state.pronunciation&&<p className="mt-3 text-xs font-semibold text-muted">Bunny nghe: “{state.transcript}”</p>}
    {state.skipped&&<p className="mt-3 text-xs font-semibold text-slate-500">Đã bỏ qua phần nói. Không tính là sai.</p>}
    {error&&<p className="mt-3 text-xs font-semibold text-amber-800">{error}{PronunciationService.configured()&&SpeechService.supported()&&<button onClick={startBrowser} className="ml-2 underline">Dùng kiểm tra từ</button>}</p>}
    {!pronunciationReady&&!SpeechService.supported()&&!paused&&<p className="mt-2 text-xs font-semibold text-amber-800">Thiết bị không hỗ trợ tự kiểm tra giọng nói. Bạn vẫn có thể tự luyện hoặc bỏ qua phần nói.</p>}
  </div>
}
