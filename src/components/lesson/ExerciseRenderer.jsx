...
function emitResult(onResult, step, correct, answer, expected) {
  onResult?.({
    correct,
    answer,
    expected,
    exerciseType: step.exerciseType,
    promptVi: step.promptVi,
    explainVi: step.explainVi,
    errorCategory: step.errorCategory,
    conceptId: step.conceptId,
  })
}

function ChoiceExercise({ step, state, onResult }) {
  ...
  const check = () => {
    ...
    const ok = normalize(picked) === normalize(step.answer)
    setChecked(true)
    emitResult(onResult, step, ok, picked, step.answer)
  }
  return <ExerciseFrame step={step} resolved={state?.resolved}>
    ...
    <ActionRow checked={checked} correct={correct} canCheck={picked != null} onCheck={check} onReset={() => { setPicked(null); setChecked(false) }}/>
    <Feedback correct={correct} text={step.explainVi}/>
  </ExerciseFrame>
}

function OrderExercise({ step, state, onResult }) {
  const resetState = () => step.tokens.map((text, i) => ({ id: i, text }))
  const [pool, setPool] = useState(resetState)
  const [built, setBuilt] = useState([])
  const [checked, setChecked] = useState(false)
  useEffect(() => { setPool(resetState()); setBuilt([]); setChecked(false) }, [step])
  ...
  const check = () => {
    ...
    const ok = normalizeLoose(answer) === normalizeLoose(step.answer)
    setChecked(true)
    emitResult(onResult, step, ok, answer, step.answer)
  }
  return <ExerciseFrame step={step} resolved={state?.resolved}>
    ...
    <ActionRow checked={checked} correct={correct} canCheck={built.length === step.tokens.length} onCheck={check} onReset={() => { setPool(resetState()); setBuilt([]); setChecked(false) }}/>
    <Feedback correct={correct} text={step.explainVi}/>
  </ExerciseFrame>
}

function TextExercise({ step, state, onResult, mode }) {
  const [value, setValue] = useState('')
  const [checked, setChecked] = useState(false)
  useEffect(() => { setValue(''); setChecked(false) }, [step])
  const accepted = Array.isArray(step.accepted) ? step.accepted : [step.answer]
  const isCorrect = () => accepted.some(a => step.flexible ? normalizeLoose(value).includes(normalizeLoose(a).replace(/[?.!]/g,'')) : normalizeLoose(value) === normalizeLoose(a))
  const correct = checked ? isCorrect() : null
  const check = () => {
    if (!value.trim()) return
    const ok = isCorrect()
    setChecked(true)
    emitResult(onResult, step, ok, value, accepted.join(' / '))
  }
  return <ExerciseFrame step={step} resolved={state?.resolved}>
    ...
    <ActionRow checked={checked} correct={correct} canCheck={!!value.trim()} onCheck={check} onReset={() => { setValue(''); setChecked(false) }}/>
    <Feedback correct={correct} text={step.explainVi}/>
  </ExerciseFrame>
}

function IdentifyExercise({ step, state, onResult }) {
  ...
  const check = () => {
    ...
    setChecked(true)
    emitResult(onResult, step, ok, selected.map(i => step.tokens[i]).join(' | '), expected.map(i => step.tokens[i]).join(' | '))
  }
  return <ExerciseFrame step={step} resolved={state?.resolved}>
    ...
    <ActionRow checked={checked} correct={correct} canCheck={selected.length>0} onCheck={check} onReset={()=>{setSelected([]);setChecked(false)}}/>
    <Feedback correct={correct} text={step.explainVi}/>
  </ExerciseFrame>
}

function ExerciseFrame({ step, resolved, children }) {
  return <div><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-black uppercase tracking-[.16em] text-violet-600">Luyện tập chủ động</p>{resolved && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-black text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5"/> ĐÃ NẮM</span>}</div><h2 className="mt-2 text-2xl font-black tracking-tight text-slate-900">{step.promptVi}</h2><div className="mt-6">{children}</div></div>
}

function ActionRow({ checked, correct, canCheck, onCheck, onReset }) {
  return <div className="mt-5 flex items-center gap-3">{!checked ? <button disabled={!canCheck} onClick={onCheck} className="rounded-2xl bg-blue-600 px-5 py-3 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-40">Kiểm tra</button> : <button onClick={onReset} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700"><RotateCcw className="h-4 w-4"/> {correct ? 'Làm lại' : 'Sửa và thử lại'}</button>}</div>
}
