...
const kindMeta={
  ...
  understand:{label:'HIỂU Ý CHÍNH',icon:Brain,cls:'text-emerald-700 bg-emerald-50 border-emerald-100'},
...
function AudioButtons({text}){
  return <div className="flex shrink-0 items-center gap-1.5"><button onClick={()=>AudioService.speak(text,{speed:'normal'})} className="pressable inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-blue-50 px-2.5 text-[11px] font-black text-blue-700" aria-label={`Nghe tốc độ bình thường: ${text}`}><Volume2 className="h-4 w-4"/> Nghe</button><button onClick={()=>AudioService.speak(text,{speed:'slow'})} className="pressable inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-slate-50 px-2.5 text-[11px] font-black text-slate-600" aria-label={`Nghe chậm: ${text}`}><Gauge className="h-4 w-4"/> Chậm</button></div>
}

function ContentStep({step}){
  const meta=kindMeta[step.kind]||kindMeta.understand;const Icon=meta.icon
  const cue={
    discover:'Nhìn ví dụ trước. Bạn nhận ra điều gì?',
    understand:'Chỉ cần nhớ một ý chính ở bước này.',
...
function ProductionStep({step,state,onStateChange}){
  ...
  return <div><div className="inline-flex items-center gap-2 rounded-full border border-amber-100 bg-amber-50 px-3 py-1.5 text-[11px] font-black tracking-[.14em] text-amber-800"><Lightbulb className="h-3.5 w-3.5"/>PRODUCE</div><h2 className="mt-4 text-[28px] font-extrabold tracking-tight">Tự tạo tiếng Anh</h2><p className="mt-3 text-[15px] font-medium leading-7 text-slate-600">{step.promptVi}</p>
